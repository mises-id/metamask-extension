...
const slice = createSlice({
  ...
  reducers: {
    updateMisesSendAmount: (state, action) => {
      ...
      draftTransaction.amount.value = amount;
      draftTransaction.amount.error =
        flag === -1 ? INSUFFICIENT_FUNDS_ERROR : null;
    ...
    updateMemo: (state, action) => {
      const draftTransaction =
        state.draftTransactions[state.currentTransactionUUID];
      draftTransaction.memo = action.payload;
      slice.caseReducers.validateSendState(state);
    },
     ...
     * @returns {void}
     */
    validateSendState: (state, action) => {
      const draftTransaction =
        state.draftTransactions[state.currentTransactionUUID];
      if (draftTransaction) {
        ...
        switch (true) {
          ...
          case new BigNumber(draftTransaction.gas.gasLimit, 16).lessThan(
            ...
            if (action && action.payload) {
              const isZero = action.payload.amount
                ? new BigNumber(action.payload.amount).comparedTo(
                    new BigNumber(0),
                  )
                : -1;
              draftTransaction.status = [-1, NaN].includes(isZero)
                ? SEND_STATUSES.INVALID
                : SEND_STATUSES.VALID;
              break;
            }
            draftTransaction.status = SEND_STATUSES.INVALID;
            console.log('INVALID111');
            break;
...
export function toggleSendMaxMode(misesGas) {
  return async (dispatch, getState) => {
    ...
    if (state[name].amountMode === AMOUNT_MODES.MAX) {
      ...
      if (state.metamask.provider.type === NETWORK_TYPES.MISES) {
...
export function getRecipient(state) {
  ...
  const account =
    accountList.find((val) => val.address === draft.recipient.address) || {};
  if (account.misesId && provider.type === NETWORK_TYPES.MISES) {
    draft.recipient.misesId = account.misesId;
  }
