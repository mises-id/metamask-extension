...
const sendMetadata = {
  methodNames: [MESSAGE_TYPE.MISES_GETADDRESSTOMISESID],
  implementation: getAddressToMisesIdHandler,
  hookNames: {
    addressToMisesId: true,
  },
};
 ...
 */
async function getAddressToMisesIdHandler(
  req,
  res,
  _next,
  end,
  ...
  const { params } = req;
  try {
    const result = await addressToMisesId(params[0]);
    res.result = result;
  } catch (error) {
    console.log(error);
    res.error = error;
